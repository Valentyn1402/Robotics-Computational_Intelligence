...
class Controller():
    ...
    #old parameters delta = 0.23, epsilon = 0.1

    def __init__(self, path: list = None, theta: float = 0, epsilon: float = 0.1, delta: float = 0.05, dt: float = 0.1):
        ...
        self.max_speed = 0.43 # m/s -> at maximum input 
        self.max_speed_angular = 0.42 # rotation speed war 0.58
        ...
        #initialize simple pid contoller alte parameter K_p = 3, K_i = 0.2, K_d = 0.01
        self.pid = PIDController(2.5, 0.2, 0.01, self.dt)
        ...
        self.u_max = 0.25 #maximum input
        self.u_min = -0.25 #minimum input
        self.u = 0
        ...
        self.rate = rospy.Rate(120) #rosopy 120
    ...
    def calculate_inputs(self, current_pos: np.array, target_point: np.array ) -> np.array: 
        ...
        pid_error = self.pid.compute(angle_error/12)
        ...
        error_neg = -(self.axle_length/12)*angle_error - pid_error
        error_pos = (self.axle_length/12)*angle_error + pid_error
        ...
        if np.pi > angle_error > self.delta:
            ...
            u_left_update = self.u_min
            u_right_update = self.u_min
    ...
    def drive_path(self) -> None:
        ...
        for coordinate in self.path:
            print("Going towards new coordinate: ", coordinate)
            ...
            while np.linalg.norm(self.pos - coordinate) > self.epsilon: 
                ...
                print("updated position in x: " + str(self.pos[0]) + " given location in y: " + str(self.pos[1]))
